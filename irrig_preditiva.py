import numpy as np
import pandas as pd
from scripts.database.conection import conectar_banco
...
class SmartIrrigationSystem:
    ...
    def coletar_dados_sensores(self):
        ...
        data = {
            'leit_p': int(np.random.uniform(0, 100)),  
            'leit_k': int(np.random.uniform(0, 80)),   
            'leit_ph': float(np.random.uniform(0.0, 14.0)),
            'leit_umidade': float(np.random.uniform(40, 80)),  # %
            'leit_temperatura': float(np.random.uniform(20, 35)),  # °C
